...
const Collision__modify = function (edit, keyEq, shift, f, h, k, size, insert, multi) {
    if (h === this.hash) {
        ...
        if (list === this.children) return list.length > 1 ? this : list[0];
...
* parent: the branch to update
* keyEq: key equality test
* edit: mutable flag
* entry: the entry to modify
* val: the value to Set
* prev: if true set prev, otherwise set next
* s: the current  height of the branch
*/
...
MapIterator.prototype.next = function () {
    ...
    if(!node) console.log(v,this.old)
    if (!node) return DONE;
    if (node.type == MULTI) {
...
*/
const toMap = map => map.fold((acc,v,k) => (acc[k] = v && v.toJS ? v.toJS() : v,acc),{});
